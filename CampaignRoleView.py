...
class CampaignRoleView(discord.ui.View):
    ...
    @discord.ui.button(label="Dungeon Master", style=discord.ButtonStyle.blurple)
    async def dm_button(self, interaction: discord.Interaction, button: discord.Button):
        ...
        if self.campaign.has_dm_already:
            ...
            if curr_user.isAdventurer:
                await interaction.response.send_message("There is already a Dungeon Master and you are an Adventurer.", ephemeral=True)
                return
            elif curr_user.isDM:
                await interaction.response.send_message(f"You already are the Dungeon Master.", ephemeral=True)
                return

            await interaction.response.send_message("There is already a Dungeon Master, choose the Adventurer role.", ephemeral=True)
        ...
        curr_user.isDM = True
        await interaction.response.send_message("You have selected Dungeon Master role.", ephemeral=True)
    ...
    @discord.ui.button(label="Adventurer", style=discord.ButtonStyle.green)
    async def adventurer_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if curr_user.isDM:
            ...
            await interaction.response.send_message("You switched from Dungeon Master to Adventurer role", ephemeral=True)

            return

        elif curr_user.isAdventurer:
            await interaction.response.send_message("You have already selected the Adventurer role", ephemeral=True)

            return

        await interaction.response.send_message("You have selected Adventurer role", ephemeral=True)
